...
export async function handleTradesByInsider(request, env) {
  ...
  let cik = cikParam ? cikParam.toString().trim() : null; // Keep as string for comparison
  ...
  if (cik) {
    // Handle both integer and decimal format CIKs
    insiderConditions.push("(person_cik = ? OR person_cik = ?)");
    insiderParams.push(cik.toString(), `${cik}.0`);
    console.log("Added CIK conditions with params:", [
      cik.toString(),
      `${cik}.0`,
    ]);
  } else if (name) {
